...
main(int argc, char *argv[])
{
    ...
    DOF  *B1, *B2, *B3, *C1, *C2, *C3, *u_p, **uhs, **FEMuhs, **coarseuhs;
...
/*********************get coarsegrid dof************/
		...
		coarseuhs = phgAlloc(numt * sizeof(*coarseuhs));
        for(i = 0; i < numt; i++ )
			coarseuhs[i] = phgDofNew(Cosg, DOF_P1, 1, "coarseuhs", DofInterpolation);

	    V1 = phgMapCreateVec(map, numt);

	    phgDofCopy(uhs[k], &coarseuhs[0], DOF_P1, "u_p1");
		...
		{	
			...
			phgFEMLoopTime3(crtime, Cosu_h, coarseuhs[i], CosC, CosG1, CosG2, Cosb);
		   	phgDofCopy(coarseuhs[i+1], &Cosu_h, DOF_P1, "u_p1");
		}
//*********************SVD1*************************/
		phgMapDofArraysToVec(map, numt, FALSE, &V1, &coarseuhs[0], NULL);
